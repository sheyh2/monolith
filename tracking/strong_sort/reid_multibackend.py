...
from tracking.torchreid.utils.tools import download_url
...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='osnet_x0_25_msmt17.pt', device=torch.device('cpu'), fp16=False):
        ...
        fp16 &= (self.pt or self.jit or self.onnx or self.engine) and device != 'cpu'  # FP16
        self.fp16 = fp16
        if self.pt:  # PyTorch
    ...
    def model_type(p='path/to/model.pt'):
        ...
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, xml2, hshs = (s in p for s in suffixes)
